document.addEventListener("DOMContentLoaded", function () {
    ...
    function f() {
        var f = document.querySelectorAll(".navlink");
        for (let k = 0; k < f.length; k++) {
            ...
            if (
                ...
                var j = window.btoa(d(g, h));
                f[k].style.backgroundImage = "url(data:image/svg+xml;base64," + j + ")";
            } else {
                6 < f[k].text.length &&
                    (f[k].innerHTML = f[k].innerHTML.replaceAll(" ", "<br>"));
                let c = a(g);
                var j = window.btoa(e("white", "black", "0.5rem", "95%"));
                f[k].style.backgroundImage = "url(data:image/svg+xml;base64," + j + ")";
                let d = -10;
                (g == u && (d = -20),
                    (f[k].style.color = b(c, d)),
                    f[k].addEventListener("mouseout", () => {
                        ((f[k].style.animationPlayState = "running"),
                            (f[k].style.color = b(c, d)),
                            (f[k].style.filter = `none`));
                    }),
                    f[k].addEventListener("mouseover", () => {
                        ((f[k].style.animationPlayState = "paused"),
                            (f[k].style.color = b(c, d + 15)),
                            (f[k].style.width =
                                parseFloat(f[k].style.width.replaceAll("vw", "")) + 0.8 + "vw"),
                            (f[k].style.filter =
                                `drop-shadow(-0.1vw -0.1vw 0.35vw ${b(c, -20)}) drop-shadow(0.1vw 0.1vw 0.35vw ${b(c)}) drop-shadow(-0.1vw -0.1vw 0.35vw white) drop-shadow(0.1vw 0.1vw 0.35vw white)`));
                    }));
                let i = 6 + h / 100;
                ((f[k].style.top = i + "vw"),
                    (f[k].style.padding = 5 - h / -250 + "vw"),
                    (document.getElementsByClassName(
                        "logo-container",
                    )[0].style.marginBottom = "-5.5vw"),
                    (document.getElementsByTagName("main")[0].style.marginTop = "-3vw"),
                    0 == k % 2
                        ? (f[k].style.setProperty("--translationStart", "0.45vw"),
                            f[k].style.setProperty("--translationEnd", "-0.45vw"),
                            (f[k].style.animation =
                                "MoveUpDown 2.5s linear infinite alternate"))
                        : (f[k].style.setProperty("--translationStart", "-0.45vw"),
                            f[k].style.setProperty("--translationEnd", "0.45vw"),
                            (f[k].style.animation =
                                "MoveUpDown 2.5s linear infinite alternate")));
    ...
    function g() {
        let a = document.getElementById("mobileNavbar");
        a.style.display = "block" === a.style.display ? "none" : "block";
    }
    function h() {
        let a = navbarHTML,
            b = document.getElementsByTagName("header")[0];
        ((b.innerHTML = headerEl),
            c()
                ? ((a = a.replaceAll("desktopNavbar", "mobileNavbar")),
                    (a = a.replaceAll("navlink", "navlink pencil")),
                    (b.id = "mobileHeader"),
                    (b.innerHTML =
                        ...
                        "</div>"))
                : (b.id = "desktopHeader"));
        let d = document.getElementsByClassName("kite-logo")[0];
        d.style.visibility = "visible";
        let e = document.getElementsByClassName("logo-img")[0];
        if (((e.hidden = !1), (b.innerHTML += a), c())) {
            let a = document.getElementById("hamburgerIcon");
    ...
    function i() {
        document.querySelectorAll("a.navlink").forEach((a) => {
            a.classList.contains("click-ready") || m(a);
            a.classList.add("click-ready");
        });
    }
    function j(a) {
        document.title =
    ...
    function l(a) {
        clearTimeout(B);
        let b = new URL(a),
            c = decodeURIComponent(b.pathname);
        if (
            (h(),
                (C.innerHTML = mainContent[c]),
                j(c),
                k(c),
            ...
            function a() {
                const b = Array.from(document.getElementsByClassName("mySlides")),
                    c = Array.from(document.getElementsByClassName("dot"));
                (b.forEach((a) => {
                    ...
                    d > b.length && (d = 1),
                    c.forEach((a) => {
                        a.classList.remove("activeDot");
                    }));
                try {
                    ((b[d - 1].style.display = "block"),
                        c[d - 1].classList.add("activeDot"),
                        clearTimeout(B),
                        (B = setTimeout(a, 7000)));
                } catch (a) {
                    clearTimeout(B);
            ...
            function b(b) {
                (clearTimeout(B),
                    (d += b),
                    d > z.length ? (d = 1) : 1 > d && (d = z.length));
                for (let a = 0; a < z.length; a++) z[a].style.display = "none";
                for (let a = 0; a < A.length; a++)
                    A[a].className = A[a].className.replace(" activeDot", "");
                ((z[d - 1].style.display = "block"),
                    (A[d - 1].className += " activeDot"),
                    (B = setTimeout(a, 7000)));
            }
            function c(b) {
                (clearTimeout(B), b > z.length ? (b = 1) : 1 > b && (b = z.length));
                for (let a = 0; a < z.length; a++) z[a].style.display = "none";
                for (let a = 0; a < A.length; a++)
                    A[a].className = A[a].className.replace(" activeDot", "");
                ((z[b - 1].style.display = "block"),
                    (A[b - 1].className += " activeDot"),
                    (B = setTimeout(a, 7000)));
            }
            for (var d = 0; B--;) clearTimeout(B);
            (function d() {
                ((z = document.getElementsByClassName("mySlides")),
                    ...
                    document
                        ...
                        .addEventListener("click", () => b(1)),
                    document.querySelectorAll(".dot").forEach((a, b) => {
                        a.addEventListener("click", () => c(b + 1));
                    }));
            })();
        } else if ("/\u043A\u043E\u043D\u0442\u0430\u043A\u0442\u0438" == c);
        const e = document.querySelectorAll(".chevronContainer");
        e.forEach((a) => {
    ...
    function n() {
        (!(function (a, c, b, d, e, f, g) {
            ...
            fbq("track", "PageView"));
        let a = document.createElement("noscript");
        ((a.innerHTML =
            '<img height="1" width="1" style="display:none" src="https://www.facebook.com/tr?id=522787340408326&ev=PageView&noscript=1" />'),
    ...
    function o() {
        ...
        ((b.async = !0),
            (b.src = "https://www.googletagmanager.com/gtag/js?id=G-65B9MZGZCH"),
            document.head.appendChild(b),
            (window.dataLayer = window.dataLayer || []),
            a("js", new Date()),
            a("config", "G-65B9MZGZCH"));
    }
    function p() {
        (n(), o());
    }
    const q =
        "\u0418\u0433\u0440\u0430\u043B\u043D\u0438\u0446\u0430 \u0417\u0430\u043D\u0438\u043C\u0430\u043B\u043D\u0438\u0446\u0430",
        r = "#e63030",
        s = "#0c69ab",
        t = "#8ab93a",
        u = "#f8cf0c",
    ...
    window.addEventListener("resize", function (a) {
        (h(),
            c()
                ? D.classList.replace("desktopBody", "mobileBody")
                : D.classList.replace("mobileBody", "desktopBody"));
        i();
    });
    const C = document.querySelector("main"),
        D = document.querySelector("body");
    c() ? D.classList.add("mobileBody") : D.classList.add("desktopBody");
    (l(window.location),
        (function a() {
            (document
            ...
            const b = document.getElementById("cookie-popup"),
                c = localStorage.getItem("cookiesAccepted");
            null === c
                ? (p(), (b.style.display = "block"))
                : "true" === c
                    ? (p(), (b.style.display = "none"))
                    : "false" === c && (b.style.display = "none");
        })());
    const E = `<p>&copy; ${new Date().getFullYear()} Игралница Занималница</p>`,
        F = document.getElementsByTagName("footer")[0];
    ((F.innerHTML = E), (F.hidden = !1));
});
