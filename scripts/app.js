...
document.addEventListener('DOMContentLoaded', function() {
    ...
    *Седмичната такса се заплаща в началото на всяка седмица, включва полудневна занималня, взимане и водене на детето от и до училище.</p>`
    ...
    function setPencilBackground() {
      const navLinks = document.querySelectorAll(".burger");
      for (let i = 0; i < navLinks.length; i++) {
        if (i===0) {
          var encodedSVG = window.btoa(pencilSVG(red, -900));
        } else if (i===1) {
          var encodedSVG = window.btoa(pencilSVG(blue, -400));
        } else if (i===2) {
          var encodedSVG = window.btoa(pencilSVG(green, -600));
        } else if (i===3) {
          var encodedSVG = window.btoa(pencilSVG(yellow, -300));
        } else if (i===4) {
          var encodedSVG = window.btoa(pencilSVG(lightblue, -200));
        } else if (i===5) {
          var encodedSVG = window.btoa(pencilSVG(orange, -600));
        } else if (i===6) {
          var encodedSVG = window.btoa(pencilSVG(purple, -100));
        } else if (i===7) {
          var encodedSVG = window.btoa(pencilSVG(darkgreen));
        } else {
          var encodedSVG = window.btoa(pencilSVG('black'));
        };
